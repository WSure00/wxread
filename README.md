...
本仓库 fork 自 [easysunny/wxread](https://github.com/easysunny/wxread)，在原作者实现的微信读书自动刷时长脚本基础上，我根据自身使用习惯做了如下定制：

## 我做的改动 🛠️

1. **推送标题优化**：成功推送的标题中带上了本次阅读时长，并去掉标题里的竖线（改用空格），通知更直观。
2. **阅读任务不再推送**：日常阅读跑完后不再每次推送消息（原推送代码以注释形式保留），改由每日检查任务统一告警，减少消息打扰。
3. **新增每日检查任务**：新增 `check.py` 与 `.github/workflows/check.yml`，每天北京时间 18:00 通过 GitHub Actions API 查询当天 wxread 工作流是否运行成功——成功则静默退出，未成功才推送失败通知。复用 `AutoRead` 环境与现有推送 secrets。
4. **稳定性小修补**：给 `READ_URL`/`FIX_SYNCKEY_URL` 等请求补上了 timeout；顺带修复了 `push.py` 中 proxies 字典类型的告警。
5. **新增 `CLAUDE.md`**：为 AI 辅助开发补充项目架构与注意事项说明。

除以上内容外，核心的阅读刷时长逻辑、签名算法、Cookie 刷新机制均保持原样。以下为原作者的完整 README：
...
5. **每日检查任务**：阅读任务本身不再推送消息（成功不打扰）。另有 `check.yml` 工作流每天北京时间 18:00 运行 `check.py`，通过 GitHub Actions API 检查今天的阅读任务是否成功执行：成功则静默，失败/未执行才推送提醒。
